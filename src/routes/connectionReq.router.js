...
connReqRouter.post('/send/:status/:toUserId', async (req, res) => {
    try {
        ...
        if (req.params.status == "interested") {
            await sendEmail(process.env.FROM_AWS_SES_EMAIL, process.env.TO_AWS_SES_EMAIL, "New Connection Request Received", "<p>"+ "Hi, You have received a new connection request from " + req.user.email+"</p>");
        }
